subroutine newton_interpolation(xu,z_r,n)

use glob_variables_cubicspl

implicit none

real xu(n)						!Given x points
real z_r(n)						!Given fx points
real newton_function			!Function to calculate the interpolation
...
do j=2,n		!Column of matrix
	...
	A(i,j)=(A(i-1,j-1)-A(i,j-1))/(xu(i-k)-xu(i))	!Equation for the triangular matrix. The k is for the x value, because we always start from x1
...
do
	write(11,*) temp, newton_function(temp)		!Start from the lowest x = xu(1) = temp and write the x and the fx (newton result)
	temp=temp+0.1								!With and increment of 0.1
	if(temp.gt.xu(n))exit						!And stop when x(n) is reached
enddo
...
real x						!The input from xu(1) to xu(n) with an increment
...
do j=1,p	
	temp_f = ((x-(x_f(j)))*(C_f(j+1)))*temp + temp_f	!READ AS: (x-x1)*C2 + 0  . The first loop temp_f should be 0, because no intermediate result
	temp = (x-(x_f(j)))*temp							!(x-x1) becomes a temp 
enddo													!-> loop 2 is:	 (x-x2) *  C3  *   (x-x1)  +   Loop 1
														!                (x-xu2)*  C(3) *    temp  +   temp_f 
newton_function = temp_c + temp_f						!-> loop 3 is:	 (x-x3) *  C4  *   (x-x1)*(x-x2) + 	Loop 1 + Loop 2
